################################################################################
##' @title build the models for our selection of shrubs species adding sum of inverse 
##'        distance to the closest occurence as predictor
##'
##' @author Damien G. 
##' @date 27/10/2015
...
#   out.dir <- "/work/georges/BRISCA/Biomod_pure_climate"
  out.dir <- "/work/georges/BRISCA/Biomod_pure_climate_invdist"
...
invdist.laea <- raster(file.path(in.dist, 
                              paste0("InvDist_", sp.bmname, ".grd")))

## merge all cliimatic variables
clim.cur <- stack(ddeg, subset(bio, c(6, 10, 18, 19)), invdist.laea)
...
rm(pres.thin.file, pres.thin, bio, ddeg, invdist.laea)
