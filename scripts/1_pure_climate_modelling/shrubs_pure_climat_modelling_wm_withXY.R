################################################################################
##' @title build the models for our selection of shrubs species adding coordinates
##'   predictor
##'
##' @author Damien G. 
##' @date 03/08/2015
...
if (host == "idiv_cluster"){
  ...
  # XY layers
  in.xy <- "/data/idiv_sdiv/brisca/Data/Climate/Macroclimate/Current/Processed/Projected/xy_raster"
  
  # output directory (= workking directory)
#   out.dir <- "/work/georges/BRISCA/Biomod_pure_climate"
  out.dir <- "/work/georges/BRISCA/Biomod_pure_climate_xy"
...
## XY coordinates
x.laea <- raster(file.path(in.xy, "x_laea.grd"))
y.laea <- raster(file.path(in.xy, "y_laea.grd"))

## merge all cliimatic variables
clim.cur <- stack(ddeg, subset(bio, c(6, 10, 18, 19)), x.laea, y.laea)
