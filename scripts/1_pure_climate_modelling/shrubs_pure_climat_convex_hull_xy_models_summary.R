...
for(sp.id in 1:189){
  ...
  if(is.new.graph){
    ...
    ## open a png device
    file.name <- file.path(out.dir, paste0("spcm_convex_hull_full_xy_models_test", graph.id, ".png"))
    cat("\n> Creating", file.name)
    png(file.name, 
  ...
  convex.hull.dir <- "~/BRISCA/workdir/brsica_shrubs_convex_hull_full"
  assign(paste0("convex.hull.poly.area", sp.panel.pos), get(load(file.path(convex.hull.dir, paste0(sp.name, "_convex_hull_poly_area.RData")))))
  assign(paste0("convex.hull.poly.group", sp.panel.pos), get(load(file.path(convex.hull.dir, paste0(sp.name, "_convex_hull_poly_group.RData")))))
  ...
  ## second graph is for convex hull over big area
  pp <- levelplot(proj.cur, par.settings = ras.theme,
                  margin = FALSE, colorkey = FALSE) + 
    eval(parse(text = paste0("layer(sp.points(pres.pts", sp.panel.pos, ", col = 'yellow', alpha = .01 ))"))) +
    eval(parse(text = paste("layer(sp.lines(convex.hull.poly.area", sp.panel.pos, "[[", 
                            1:length(get(paste0("convex.hull.poly.area", sp.panel.pos))),
                            "]], col = 'red'))", sep = "", collapse = " + ")))
  if(sp.panel.pos == 1) pp$main <- "current proj\n+ CH area"
  ## add the graph in the list
  lis.args[[(sp.panel.pos-1) * panel.ncol + 2]] <- pp
  
  ## third graph is for convex hull build with automatic clustering
  pp <- levelplot(proj.cur, par.settings = ras.theme,
                  margin = FALSE, colorkey = FALSE) + 
    eval(parse(text = paste0("layer(sp.points(pres.pts", sp.panel.pos, ", col = 'yellow', alpha = .01 ))"))) +
    eval(parse(text = paste("layer(sp.lines(convex.hull.poly.group", sp.panel.pos, "[[", 
                            1:length(get(paste0("convex.hull.poly.group", sp.panel.pos))),
                            "]], col = 'blue'))", sep = "", collapse = " + ")))
  if(sp.panel.pos == 1) pp$main <- "current proj\n+ CH clust"
  ## add the graph in the list
  lis.args[[(sp.panel.pos-1) * panel.ncol + 3]] <- pp
  
  ## fourth graph is for convex hull over big area coupled with automatic clustering
  pp <- levelplot(proj.cur, par.settings = ras.theme,
                  margin = FALSE, colorkey = FALSE) + 
    eval(parse(text = paste0("layer(sp.points(pres.pts", sp.panel.pos, ", col = 'yellow', alpha = .01 ))"))) +
