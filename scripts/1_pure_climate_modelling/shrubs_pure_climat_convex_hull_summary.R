...
for(sp.id in 1:189){
  ...
  ## -- create a new png file each 25 species ------------------------------------
  is.new.graph <- ((sp.id - 1) %% (panel.ncol * panel.nrow)) == 0
  if(is.new.graph){
    graph.id <- (sp.id %/% (panel.ncol * panel.nrow)) + 1
    ## open a png device
    png(file.path(out.dir, paste0("spcm_convex_hull_test_", graph.id, ".png")), 
        width = img.width, height = img.height, res = 80, type = 'cairo')
  }
  
  ## get the pannel position of the species
  sp.panel.pos <- (sp.id) %% (panel.ncol * panel.nrow)
  if(sp.panel.pos == 0) sp.panel.pos <- (panel.ncol * panel.nrow)
  ...
  lis.args[[sp.panel.pos]] <- pp
  
  is.last.panel <- (sp.panel.pos == (panel.ncol * panel.nrow) | sp.id == 189)
  if(is.last.panel){
    lis.args <- c( lis.args, ncol = panel.ncol, main = list(textGrob(paste0("\nConvex Hull filtering test",
