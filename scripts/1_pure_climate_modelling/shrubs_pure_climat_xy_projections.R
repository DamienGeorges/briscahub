...
#   in.mod <- "/work/georges/BRISCA/Biomod_pure_climate"
  in.mod <- "/work/georges/BRISCA/Biomod_pure_climate_xy"
...
## XY coordinates
x.laea <- raster(file.path(path.to.expl.var, "xy_raster", "x_laea.grd"))
y.laea <- raster(file.path(path.to.expl.var, "xy_raster", "y_laea.grd"))

## merge all cliimatic variables
expl.stk <- stack(ddeg, subset(bio, c(6, 10, 18, 19)), x.laea, y.laea)
