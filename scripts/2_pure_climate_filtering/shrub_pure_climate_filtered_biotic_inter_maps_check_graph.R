...
in.dir.biotic <- "/work/georges/BRISCA/Biomod_biotic_interaction_maps_2017-04-06"
...
## temporarly remove couple of species
# sp.list <- sp.list[-69]

nb.pdf <- ceiling(length(sp.list)/20) 

for(pdfid in 1:nb.pdf){
  cat("\n> new pdf (", pdfid,")_________")
  pdf(paste0("check_biotic_interaction_maps_", sprintf("%02d", pdfid), ".pdf"))
  for(sp_ in sp.list[((pdfid - 1)*nb.plot.by.pdf + 1): min(((pdfid)*nb.plot.by.pdf), length(sp.list))]){
