...
out.dir <- "/work/georges/BRISCA/Biomod_pure_climate_filtered_final_summary"
...
alpha.div.stk <- height.sum.stk <- height.max.stk <- sp.height.stk <- raster("/data/idiv_sdiv/brisca/Data/no_interaction_mask.grd")
...
for(sp.id in 1:nrow(sp.tab)){ ## loop over species
  ...
  ##cat(ldir)
  sp.proj.files  <- sapply(ldir, function(x) list.files(file.path(x,"individual_projections"), pattern = "EMwmeanByTSS_mergedAlgo_mergedRun_mergedData_TSSbin_filt_ch.grd", full.names = TRUE))
  ...
  ## fill points of maps that have been removed during filtering
  sp.proj.stk[is.na(sp.proj.stk)] <- 0 ## trick to be able to work on the full area
  ...
  ## -- add this height stack to the current one -------------------------------
    ...
    if(sp.id == 1){ height.max.stk <- height.sum.stk} else{
...
save.image(file.path(out.dir, "spcfhmc_backup.RData"))
