...
#   if (is.null(temp_workdir)) temp_workdir <- paste("maxentWDtmp", format(Sys.time(), "%s"), sep="")
  ...
  for(spl in 1:split.proj){
    cat("\n\t\t> projecting block", spl, "on",  split.proj)
    ## TODO(DAMIEN) create a java option option for maxent
    system(command=paste("java ", ifelse(is.null(object@model_options$memory_allocated),"",paste0("-mx", object@model_options$memory_allocated, "m")),
